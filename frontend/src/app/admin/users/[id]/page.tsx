...
export default function UserDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchUserDetail = async () => {
    try {
      setLoading(true);
      const response = await authenticatedFetch(`${getUsersUrl()}/${params.id}`);
  ...
  const handleToggleRole = async () => {
    ...
    const confirmMessage = `Энэ хэрэглэгчийг ${newRole === "admin" ? "админ" : "энгийн хэрэглэгч"} болгохдоо итгэлтэй байна уу?`;
    
    if (confirm(confirmMessage)) {
      try {
        setUpdating(true);
        const response = await authenticatedFetch(`${getUsersUrl()}/${userDetail.id}/role`, {
          method: "PATCH",
  ...
  const handleToggleHighlight = async () => {
    ...
    try {
      setUpdating(true);
      const response = await authenticatedFetch(`${getUsersUrl()}/${userDetail.id}/highlight`, {
        method: "PATCH",
      });
  ...
  const handleDeleteUser = async () => {
    ...
    if (confirm(confirmMessage)) {
      try {
        setUpdating(true);
        const response = await authenticatedFetch(`${getUsersUrl()}/${userDetail.id}`, {
          method: "DELETE",
        });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Хэрэглэгчийн мэдээлэл ачааллаж байна...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Хэрэглэгчийн дэлгэрэнгүй</h1>
                <p className="text-sm text-gray-600">Хэрэглэгчийн бүрэн мэдээлэл харах, засах</p>
              </div>
            </div>
            <div className="text-sm text-gray-600">
              Админ: {user.firstName}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Хэрэглэгчийн ID</label>
                  <div className="flex items-center space-x-3">
                    {userDetail.userCode ? (
                      <>
                        ...
                        <button
                          onClick={() => navigator.clipboard.writeText(userDetail.userCode!)}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Бүртгүүлсэн огноо</label>
                  <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-2">
                    {new Date(userDetail.createdAt).toLocaleDateString("mn-MN", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                      hour: "2-digit",
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Удирдлагын үйлдэл</h3>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <span>
                    {userDetail.role === "admin" ? "Энгийн хэрэглэгч болгох" : "Админ болгох"}
                  </span>
                </button>
            ...
            {/* Stats Card */}
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Статистик</h3>
              
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Сүүлд өөрчлөгдсөн:</span>
